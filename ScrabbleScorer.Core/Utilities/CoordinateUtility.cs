...
{
    public static bool CanMakeWordOnCoordinate(Coordinate[] occupiedCoordinates, Coordinate coordinate, Alignment alignment, int wordLength) 
    {
        ...
        while (true) 
        {
            ...
            if (!hasEmptyCoordinate
                && !occupiedCoordinates.Contains(currCoordinate))
                hasEmptyCoordinate = true;

            isAdjacentToOtherWord = isAdjacentToOtherWord 
        ...
        return isWithinBoardDimensions
            && isAdjacentToOtherWord
            && hasEmptyCoordinate;
    }

    private static bool IsAdjacentCoordinateHasLetter(Coordinate[] occupiedCoordinates, Coordinate coordinate) =>
        occupiedCoordinates
            .Any(c => c == new Coordinate(coordinate.X + 1, coordinate.Y)
